 ...
 */
walletRouter.post("/data", async (req: Request, res: Response) => {
  try {
 ...
 */
walletRouter.post("/tokens", async (req: Request, res: Response) => {
  try {
    ...
    const tokens = await getERC20TokenBalances(
      address,
      chainId,
      tokenAddresses,
    );
 ...
 * @param {number} chainId - Chain ID
 */
walletRouter.post("/balance", async (req: Request, res: Response) => {
  try {
    const { address, chainId = 1 } = req.body;
 ...
 */
walletRouter.post("/multi-chain", async (req: Request, res: Response) => {
  try {
    ...
    Object.values(multiChainData).forEach((chainData: any) => {
