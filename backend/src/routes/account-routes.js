 ...
 */
accountRouter.get("/profile", ensureAuthenticated, (req, res) => {
    try {
 ...
 */
accountRouter.put("/profile", ensureAuthenticated, (req, res) => {
    try {
        const userId = req.session?.user?.id;
        if (!userId) {
            return res.status(401).json({
 ...
 *         description: Unauthorized
 */
accountRouter.get("/preferences", ensureAuthenticated, (req, res) => {
    try {
        const userId = req.session?.user?.id;
        if (!userId) {
            return res.status(401).json({
 ...
 */
accountRouter.put("/preferences", ensureAuthenticated, (req, res) => {
    try {
        const userId = req.session?.user?.id;
        if (!userId) {
            return res.status(401).json({
        ...
        const { theme, language, currency, emailNotifications, pushNotifications } = req.body;
        // Validate theme
 ...
 */
accountRouter.post("/deactivate", ensureAuthenticated, (req, res) => {
    try {
        const userId = req.session?.user?.id;
        if (!userId) {
            return res.status(401).json({
