...
import { getCached, setCached } from "../utils/redis.ts";
...
export async function introspectToken(
  accessToken: string,
): Promise<TokenIntrospectionResult | null> {
  const cacheKey = `token:introspect:${accessToken}`;

  // Try cache first
  const cached = await getCached<TokenIntrospectionResult>(cacheKey);
  if (cached !== null) {
  ...
  try {
    ...
    const response = await axios.post<TokenIntrospectionResult>(
      `${config.oauth.serverUrl}/oauth2/introspect`,
      new URLSearchParams({
        token: accessToken,
    ...
    logger.error(
      "Token introspection failed - falling back to session validation",
      {
        error: error.message,
        code: error.code,
