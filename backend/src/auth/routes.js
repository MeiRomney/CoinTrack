...
import { getAuthProvider, isMockAuthEnabled } from "./provider-factory.js";
...
export function ensureRole(requiredRoles) {
...
authRouter.post("/logout", async (req, res) => {
    if (req.session.accessToken && req.session.refreshToken) {
        try {
            ...
            logger.error("Provider logout failed:", error);
        }
    }
 ...
 */
authRouter.get("/mock/users", async (req, res) => {
    if (!isMockAuthEnabled()) {
        res.status(403).json({
            error: "forbidden",
            message: "Mock authentication is disabled",
