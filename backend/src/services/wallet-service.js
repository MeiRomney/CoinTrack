...
export async function getERC20TokenBalances(address, chainId, tokenAddresses) {
    try {
        const config = CHAIN_CONFIGS[chainId];
        if (!config) {
            throw new Error(`Unsupported chain: ${chainId}`);
...
async function getTokensFromEtherscan(address, chainId, config) {
    try {
        if (!config.etherscanApiKey) {
            console.warn(`No API key for chain ${chainId}, returning empty token list`);
            return [];
        }
        ...
        const tokenAddresses = Array.from(new Set(response.data.result.map((tx) => tx.contractAddress)));
...
export async function getMultiChainWalletData(address, chainIds = [1, 137, 42161, 10]) {
    try {
        const results = {};
