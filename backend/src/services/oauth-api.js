...
class OAuthApiClient {
    ...
    async getAccessToken() {
        ...
        try {
            ...
            const clientSecret = config.oauth.serviceClientSecret || config.oauth.clientSecret;
            const response = await axios.post(`${config.oauth.serverUrl}/oauth2/token`, new URLSearchParams({
                grant_type: "client_credentials",
                client_id: clientId,
                client_secret: clientSecret,
                scope: "auth:organization:users auth:organization:users:create auth:organization:users:modify:roles auth:organization:roles",
            }).toString(), {
                headers: {
     ...
     */
    orgUsersCache = null;
    ORG_USERS_CACHE_TTL = 5000; // 5 seconds
    async getOrgUsersWithRoles(organizationId) {
        // Return cached if valid
        if (this.orgUsersCache &&
     ...
     * @param organizationId - The organization ID (required for client credentials auth)
     */
    async getUserRolesWithIds(userId, organizationId) {
        try {
            const users = await this.getOrgUsersWithRoles(organizationId);
            const user = users.find((u) => u.id === userId);
            if (!user) {
     ...
     * @param organizationId - The organization ID (required for client credentials auth)
     */
    async getUsersRolesBatch(userIds, organizationId) {
        const results = {};
        try {
    ...
    async getOrganizationRoles(organizationId) {
        ...
        try {
            ...
            return (response.data.roles || []).map((r) => ({
                id: r.id,
                name: r.name,
            }));
        }
     ...
     */
    async updateUserRBAC(userId, data) {
        const token = await this.getAccessToken();
        try {
     ...
     */
    async updateUserTeamAndRole(userId, newTeamRoleName, newUserRoleName, organizationId, currentRoles) {
        ...
        const otherRoleIds = currentRoles
            .filter((r) => !r.roleName.startsWith("university:team:") &&
     ...
     */
    async createUserWithTeamAndRole(input, teamRoleName, userRoleName) {
        ...
        if (teamRoleName) {
            ...
            if (teamRole) {
                ...
                logger.warn(`Team role not found during user creation: ${teamRoleName}`);
            }
        }
        if (userRoleName) {
            ...
            if (userRole) {
                ...
                logger.warn(`User role not found during user creation: ${userRoleName}`);
            }
        }
