...
export async function publishEvent(payload) {
    if (!EVENTS_SERVER_URL || !EVENTS_API_KEY) {
        ...
        if (process.env.NODE_ENV === "development") {
            console.warn("[Event Publish Skipped] EVENTS_SERVER_URL/EVENTS_API_KEY not configured", payload.event);
            return null;
        }
        throw new Error("EVENTS_SERVER_URL and EVENTS_API_KEY must be set in environment variables");
    }
    try {
...
//# sourceMappingURL=eventsPublisher.js.map